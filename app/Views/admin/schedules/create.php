...
<div class="p-4 sm:p-6">
    ...
    <div class="mb-8">
        <div class="flex flex-col lg:flex-row lg:justify-between lg:items-start mb-6">
            <div>
                ...
                <p class="mt-2 text-sm lg:text-base text-gray-600">Create a new schedule entry</p>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="p-6">
            <form id="createScheduleForm" action="<?= base_url('admin/schedules/store') ?>" method="POST">
                <?= csrf_field() ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="academic_year_id" name="academic_year_id" required class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <?php foreach ($academicYears as $year): ?>
                                <option value="<?= $year['id'] ?>"><?= esc($year['name']) ?><?= $year['is_current'] ? ' (Current)' : '' ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="subject_id" name="subject_id" required class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <?php foreach ($subjects as $subject): ?>
                                <option value="<?= $subject['id'] ?>"><?= esc($subject['name']) ?> (<?= esc($subject['code']) ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="time" id="start_time" name="start_time" required class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>

                    <!-- End Time -->
                    <div>
                        <label for="end_time" class="block text-sm font-medium text-gray-700 mb-2">End Time</label>
                        <input type="time" id="end_time" name="end_time" required class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>

                    <!-- Room -->
                    <div>
                        <label for="room" class="block text-sm font-medium text-gray-700 mb-2">Room</label>
                        <input type="text" id="room" name="room" placeholder="Enter room name/number" maxlength="100" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>

                    <!-- Notes -->
                    <div class="md:col-span-2">
                        <label for="notes" class="block text-sm font-medium text-gray-700 mb-2">Notes</label>
                        <textarea id="notes" name="notes" rows="3" placeholder="Additional notes (optional)" maxlength="500" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none"></textarea>
                    </div>
                </div>
...
<div id="loadingOverlay" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-lg p-6 max-w-sm w-full mx-4">
        <div class="flex items-center">
            ...
            <div class="ml-4">
                <h3 class="text-lg font-medium text-gray-900">Creating Schedule</h3>
                ...
                fetch(form.action, {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            showAlert(data.message || 'Failed to create schedule', 'error');
                            if (data.errors) {
                    ...
                    .catch(error => {
                        ...
                        loadingOverlay.classList.add('hidden');
                        showAlert('An error occurred while creating the schedule', 'error');
                    })
                    .finally(() => {
